...
(function() {
  ...
  function createOrder(items, address, estimatedTime, totalSavings, paymentMethodId) {
    ...
    items.forEach(function(item) {
      ...
      var menuItem = allMenuItems.find(function(mi) { return mi.id === item.menu_item_id; });
      if (menuItem) {
        var restaurant = allRestaurants.find(function(r) { return r.id === menuItem.restaurant_id; });
  ...
  function showOrdersAndSavings() {
    ...
    // Calculate current orders total
    ...
    pendingOrders.forEach(function(order) {
      ...
      currentOrdersSavings += order.total_savings || 0;
    });
    ...
    // Edit order buttons
    box.querySelectorAll('.edit-order-btn').forEach(function(btn) {
      btn.addEventListener('click', function() {
    ...
    // Track order buttons
    box.querySelectorAll('.track-order-btn').forEach(function(btn) {
      btn.addEventListener('click', function() {
        var orderId = parseInt(btn.getAttribute('data-order-id'));
    ...
    box.querySelectorAll('.delete-order-btn').forEach(function(btn) {
      btn.addEventListener('click', function() {
        var orderId = parseInt(btn.getAttribute('data-order-id'));
  ...
  function showOrderHistory() {
    ...
    function attachOrderHistoryListeners() {
      box.querySelectorAll('.cancel-order-btn').forEach(function(btn) {
        btn.addEventListener('click', function() {
          var orderId = parseInt(btn.getAttribute('data-order-id'));
    ...
    function renderList() {
      var rows = restaurants.map(function(r){
        return '<div style="display:flex;align-items:center;gap:12px;padding:8px 0;border-bottom:1px solid #eee;">' +
          ...
          '<div style="font-size:12px;color:#666;">'+ (r.area||'-') +' · '+ (r.cuisine||'-') +' · '+ (r.price_level||'$') + (r.halal ? ' · 🕌 Halal' : '') +'</div>'+ 
  ...
  function initOwnerDashboardPage() {
    try {
      ...
      function getOwnerRestaurantIds() {
        return getRestaurants().filter(function(r){ return r.owner_id === user.id; }).map(function(r){ return r.id; });
      }
      function getAllOrders() { return getStorage(STORAGE_ORDERS, []) || []; }
      function getRestaurantById(restId) {
        return getRestaurants().find(function(r){ return r.id === restId; });
      }
      ...
      function getOwnerOrders() {
        ...
        var allItems = getStorage(STORAGE_MENU_ITEMS, []) || [];
        var menuById = {};
        allItems.forEach(function(it){ menuById[it.id] = it; });
        ...
        orders.forEach(function(o){
          ...
          (o.items||[]).forEach(function(line){
            var menu = menuById[line.menu_item_id];
            if (menu && restIds.indexOf(menu.restaurant_id) !== -1) {
      ...
      function renderList() {
        var rows = restaurants.map(function(r){
          ...
          return '<div class="restaurant-item" data-rest-id="'+r.id+'" style="margin-bottom:15px;background:linear-gradient(135deg, #fff 0%, #f9f9f9 100%);border:2px solid #eee;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.05);overflow:hidden;">' +
            ...
            '<div style="font-size:13px;color:#666;margin-bottom:3px;"><span style="color:#ff6f61;">📍</span> '+ (r.area||'-') +' · <span style="color:#ff6f61;">🍴</span> '+ (r.cuisine||'-') +' · <span style="color:#ff6f61;">💰</span> '+ (r.price_level||'¥') +'</div>'+ 
  ...
  function initOrdering() {
    ...
    if (!addButtons || !addButtons.length) return;
    ...
    function showCartModal() {
      ...
      function attachCartListeners() {
        ...
        box.querySelectorAll('.cart-remove-item').forEach(function(btn) {
          btn.addEventListener('click', function() {
            var index = parseInt(btn.getAttribute('data-index'));
            if (currentCart[index] && confirm('Remove ' + currentCart[index].name + ' from cart?')) {
              currentCart.splice(index, 1);
              // Save cart immediately
              setCart(currentCart);
              updateCartButton();
      ...
      box.querySelector('#mmCartPlace').onclick = function(){
        var user = getCurrentUser(); if (!user) { window.location.href = 'login.html'; return; }
        var address = addressInput.value.trim();
        if (!address) {
        ...
        if (!paymentMethodSelect || !paymentMethodSelect.value) {
          alert('Please select a payment method or add one first.');
          if (addPaymentLink) {
            addPaymentLink.scrollIntoView({ behavior: 'smooth', block: 'center' });
          }
          return;
        }
        ...
        var selectedPaymentMethodId = paymentMethodSelect ? paymentMethodSelect.value : null;
  ...
  function showEditOrder(orderId) {
    ...
    function attachEditOrderListeners() {
      box.querySelectorAll('.qty-inc').forEach(function(btn) {
      btn.addEventListener('click', function() {
        var index = parseInt(btn.getAttribute('data-index'));
      ...
      box.querySelectorAll('.qty-dec').forEach(function(btn) {
        btn.addEventListener('click', function() {
          var index = parseInt(btn.getAttribute('data-index'));
      ...
      box.querySelectorAll('.remove-item').forEach(function(btn) {
        btn.addEventListener('click', function() {
          var index = parseInt(btn.getAttribute('data-index'));
    ...
    function render(restaurants) {
      ...
      restaurants.forEach(function(r) {
        ...
        imgWrap.style.cssText = 'background-color:#ffffff;height:150px;border-radius:15px;margin-bottom:15px;overflow:hidden;display:flex;justify-content:center;align-items:center;';
          ...
          // Make the image clickable by wrapping with an anchor when we know the page
          var linkUrl = null;
          if (r.link) linkUrl = r.link;
          else if (r.id === 'lawson') linkUrl = './lawson.html';
          else if (r.id === 'gyomu') linkUrl = './Gyomu.html';
            else if (r.id === 'marushoku') linkUrl = './marushoku.html';
            else if (r.id) linkUrl = './restaurant.html?rid=' + encodeURIComponent(r.id);
        ...
        p.textContent = 'Area: ' + (r.area || '-') + ' | Type: ' + (r.cuisine || '-') + ' | ' + (r.price_level || '¥');
  ...
  function initOrdersPage() {
    ...
    if (content) {
      ...
      // Track order buttons
      document.querySelectorAll('.track-order-btn').forEach(function(btn) {
        btn.addEventListener('click', function() {
          var orderId = parseInt(btn.getAttribute('data-order-id'));
          var order = pendingOrders.find(function(o) { return o.id === orderId; });
          if (order) {
